...
module.exports= function({userRepository}){
    ...
    exports.updateUser = function(newUser,auth, callback){
        const errors = userAuthentication.validateProfile(newUser, callback)
        if(errors.length > 0){
            callback(errors,null)
            return
        }
        if(auth.userId === auth.authorId){
            profileRepository.updateUserById(newProfile, callback)
        }
    ...
    exports.deleteProfile = function(id,auth, callback){        
        if(auth.userId === auth.authorId){
            profileRepository.deleteProfileByUserId(id, callback)
        }
        else callback(['notLoggedIn'], null)
    }
