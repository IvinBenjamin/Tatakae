...
module.exports= function({userRepository}){
    ...
    exports.updateUser = function(username, id, auth, callback){
        const errors = userValidator.validateUser(username, callback)
        if(errors.length > 0){
            callback(errors,null)
            return
        }
        if(auth.userId === auth.authorId){
            userRepository.updateUserById(username, auth.userID, callback)
        }
